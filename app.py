...
st.set_page_config(page_title="Validador de RELATÓRIOS (Excel)", layout="wide")
...
try:
    ...
    st.warning(f"Logo não carregado ({logo_file}): {e}")
...
def is_probable_header_cell(x: Any) -> bool:
    ...
    if not s:
        return False
    # Se for basicamente número/tempo/pontuação, assume que não é rótulo
    tmp = s.replace(".", "").replace(",", "").replace("/", "").replace("-", "").replace(":", "")
    if tmp.isdigit():
        return False
    # Se tiver alguma letra, tem cara de cabeçalho
    return any(ch.isalpha() for ch in s)

@st.cache_data(show_spinner=False)
def detect_header_row_simple(df_raw: pd.DataFrame, max_scan: int = 30) -> Optional[int]:
    """Acha a primeira linha com traços de cabeçalho e uma linha seguinte plausível como dados."""
    ...
    for i in range(n):
        ...
        if vals:
            ...
            if unique_ratio < 0.6:
                continue
        # próxima linha com preenchimento coerente
        if i + 1 < len(df_raw):
...
def detect_type_mixtures(df: pd.DataFrame, force_text: Optional[Set[str]] = None, force_numeric: Optional[Set[str]] = None) -> pd.DataFrame:
    ...
    for c in df.columns:
        ...
        nname = norm_text(c)
        # 1) Identificadores (forçar TEXT)
        ID_HINTS = ["_ID_NORM", "ID", "CHAVE", "REQ", "REQUER", "SOLICIT", "PROTOC"]
        if (nname in norm_force_text) or any(h in nname for h in ID_HINTS):
            # É um identificador (ex.: REQUERIMENTO, _ID_NORM, SOLICITACAO_REQUERIMENTO) -> tratar como texto
            # Não reportamos mistura para IDs
            continue

        # 1b) Coordenadas (forçar NUMERICO)
        COORD_HINTS = ["LATITUDE", "LONGITUDE", "COORD", "SIRGAS"]
        if (nname in norm_force_num) or any(h in nname for h in COORD_HINTS):
            expected_forced_numeric = True
        else:
            expected_forced_numeric = False

        # 2) Sinais auxiliares
        has_letters = nonempty.str.contains(r"[A-Za-z]", regex=True, na=False)

        # 3) Números (normaliza vírgula como decimal e remove símbolos)
        ...
        if expected == "DATA":
            ...
            has_dot = nonempty.str.contains("\.", na=False)
...
def format_dates_for_export(df: pd.DataFrame, preferred_cols: Optional[List[str]] = None) -> pd.DataFrame:
    out = df.copy()
    # 1) Colunas explicitamente passadas
    cols = set(preferred_cols or [])
    # 2) Heurística: campos com "DATA" no nome ou dtype datetime64
    for c in out.columns:
        if c in cols or ("DATA" in norm_text(c) or str(out[c].dtype).startswith("datetime64")):
            try:
                ...
                # Se não der pra converter, mantém como está
                pass
    return out
...
def _count_empty(series: pd.Series) -> int:
    try:
        s = series.astype(str).str.strip()
        return int(s.replace({"": pd.NA}).isna().sum())
    except Exception:
...
def render_base_cards(df: pd.DataFrame, base_nome: str, pub_col: Optional[str], uso_col: Optional[str], fin_col: Optional[str]):
    ...
    # Helpers
    def _count_empty_local(series: pd.Series) -> int:
        try:
            s = series.astype(str).str.strip()
            return int(s.replace({"": pd.NA}).isna().sum())
        except Exception:
    ...
    # Todos os vazios por coluna (em grade)
...
# Cards por TABELA específica (Somente A/B etc.)
...
def build_checklist(df_a: pd.DataFrame, df_b: pd.DataFrame) -> pd.DataFrame:
    """Checklist coluna-a-coluna entre A e B (presença, dtypes, preenchimento e matching por _ID_NORM).
    Robusto para: (1) IDs duplicados em linhas e (2) **colunas duplicadas** com o mesmo nome.
    Para comparar valores, reduzimos a 1 valor por ID (primeiro não-nulo normalizado).
    """
    def _collapse_dup_col(df: pd.DataFrame, colname: str) -> Optional[pd.Series]:
        ...
        if mask.sum() == 1:
            s = df[colname]
            return s
        # Se houver colunas duplicadas com o mesmo nome, escolhe o primeiro não-nulo por linha
        sub = df.loc[:, mask]
        # preenche para a direita e pega a 1ª coluna resultante
        # (alternativa à apply linha-a-linha, muito mais rápida)
    ...
    for c in cols_union:
        ...
        if has_id and in_a and in_b:
            # Colapsa _ID_NORM (caso haja colunas duplicadas) e o valor da coluna alvo
            ...
            if idA is not None and idB is not None and valA is not None and valB is not None:
                tmp_a = pd.DataFrame({"_ID_NORM": idA, "VAL": valA}).dropna(subset=["_ID_NORM"])  # mantém VAL nulo (filtramos depois)
                tmp_b = pd.DataFrame({"_ID_NORM": idB, "VAL": valB}).dropna(subset=["_ID_NORM"])  # idem

                # Contagem de IDs duplicados (visibilidade no checklist)
    ...
    # Ordena: colunas existentes em ambas primeiro; depois menor % de preenchimento
...
def generate_checklist_pdf(df: pd.DataFrame) -> Optional[bytes]:
    """Gera um PDF simples com a tabela de checklist. Retorna bytes ou None se não for possível."""
    try:
...
try:
    ...
    # Identificar automaticamente colunas de datas mesmo com legendas/imagens antes
    ...
    # Detecta USO/FINALIDADE e renderiza cards por base
    ...
    st.subheader("Misturas de tipo — Base A")
    ...
    st.subheader("Misturas de tipo — Base B")
    ...
    if "_ID_NORM" in df_a.columns and "_ID_NORM" in df_b.columns:
        ...
        with col3:
            ...
            # CSV já com datas formatadas dd/MM/yyyy
        ...
        merged = df_a.merge(df_b.add_prefix("B__"), left_on="_ID_NORM", right_on="B__" + "_ID_NORM", how="left", indicator=True)
                        # Exporta o merge com datas no padrão dd/MM/yyyy
        export_merged = format_dates_for_export(merged)
        
        # ================== COMPARAÇÃO LADO A LADO (A vs B) ==================
        ...
        if not common_cols:
            ...
            # Formata datas dd/MM/yyyy nas duas versões das colunas
